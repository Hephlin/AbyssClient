...
extends Class163 {
    public Class163 class163;
    public Minecraft mc;
    public Class128 class128;
        ...
        if (stringArray[1].toLowerCase().startsWith("a")) {
            if (stringArray.length >= 4) {
                try {
                    ...
                    Method663(class128, String.format("Added %s as alias %s.", stringArray[2], stringArray[3]));
            ...
            if (stringArray.length >= 3) {
                if (Manager.Field489.Method198(stringArray[2])) {
                    this.Method663(class128, String.format("Removed %s in alias.", stringArray[2]));
                    this.Method663(Class128.Warning, String.format("Removed %s in alias.", stringArray[2]));
                } else {
                    this.Method663(class128, String.format("%s doesn't have an alias.", stringArray[2]));
    ...
    public void Method662(String name) {
        mc.player.sendChatMessage(name);
    }
}
