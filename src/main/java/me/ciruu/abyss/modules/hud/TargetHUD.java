...
extends Module {
    ...
    @EventHandler
    private Listener Field1387 = new Listener<Class35>(this::getEnable, new Predicate[0]);
    ...
    private void Method1792() {
        ...
        if (entityLivingBase == null || entityLivingBase.isDead)return;
    ...
    private EntityLivingBase Method1793() {
        return ((AutoCrystal)Manager.moduleManager.getModuleByClass(AutoCrystal.class)).player != null ? ((AutoCrystal)Manager.moduleManager.getModuleByClass(AutoCrystal.class)).player : (KillAura.Field1388 instanceof EntityLivingBase ? (EntityLivingBase)KillAura.Field1388 : this.Method1799());
    }
    ...
    private EntityLivingBase Method1799() {
        ...
        for (Entity entity : Globals.mc.world.loadedEntityList) {
            if (entity instanceof Entity) {
                if (entity == Globals.mc.player || !(entity instanceof EntityLivingBase) || Manager.Field223.Method711(entity) || entity.isDead || entityLivingBase != null && Minecraft.getMinecraft().player.getDistance(entity) > Minecraft.getMinecraft().player.getDistance((Entity) entityLivingBase))
                    continue;
