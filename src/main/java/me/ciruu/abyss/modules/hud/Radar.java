...
extends Module {
    ...
    @EventHandler
    private Listener Field3176 = new Listener<Class35>(this::getEnable, new Predicate[0]);

    public Radar() {
        super("Radar", "Displays nearby entities in a radar.", Category.HUD, "");
    ...
    private void getEnable(Class35 class35) {
        ...
        for (EntityPlayer entityPlayer : Globals.mc.world.playerEntities) {
            if (entityPlayer instanceof EntityPlayer) {
                ...
                if (Math.abs(f3) + (float) ((Integer) this.radius.getValue()).intValue() > (float) ((Integer) this.width.getValue()).intValue() / 2.0f || Math.abs(f4) + (float) ((Integer) this.radius.getValue()).intValue() > (float) ((Integer) this.height.getValue()).intValue() / 2.0f)
                    continue;
