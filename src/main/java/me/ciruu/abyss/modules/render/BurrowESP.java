...
extends Module {
    ...
    public BurrowESP() {
        super("BurrowESP", "Highlights the blocks that people are inside.", Category.RENDER, "");
    ...
    private void Method1934() {
        for (EntityPlayer entityPlayer : Globals.mc.world.playerEntities) {
            if (entityPlayer instanceof EntityPlayer) {
                if (Globals.mc.player.getDistance((Entity) entityPlayer) > (float) ((Integer) this.radius.getValue()).intValue() || ((Boolean) this.ignoreself.getValue()).booleanValue() && entityPlayer == Globals.mc.player)
                    continue;
                ...
                if (((Boolean) this.onlyobsidian.getValue()).booleanValue() || Globals.mc.world.getBlockState(Class30.Method209(entityPlayer)).getBlock().isReplaceable((IBlockAccess) Globals.mc.world, Class30.Method209(entityPlayer)))
                    continue;
                Class50.Method137(Class30.Method209(entityPlayer), (Color) this.color.getValue(), false, (Color) this.color.getValue(), 1.0f, true, true, ((Color) this.color.getValue()).getAlpha(), false);
            }
        }
