...
extends Module {
    ...
    private final Timer Field3485 = new Timer();
    ...
    public Class304() {
        ...
        this.addSetting(this.Field3472);
        this.addSetting(this.Field3473);
        this.addSetting(this.Field3474);
        this.addSetting(this.Field3475);
        this.addSetting(this.Field3476);
    }

    public void Method4221() {
        super.getEnable();
        MinecraftForge.EVENT_BUS.register((Object)this);
        this.Field3485.reset();
        this.Field3480 = 4;
        this.Field3481 = 0;
        this.Field3477 = 0;
        ...
        if (((Boolean)this.Field3476.getValue()).booleanValue() && !Manager.moduleManager.isModuleEnabled(Step.class) && ((Step)Manager.moduleManager.getModuleByClass(Step.class)).mode.getValue() == Class393.Vanilla) {
            ((Step)Manager.moduleManager.getModuleByClass(Step.class)).enable();
        }
    }
    ...
    private void Method4227(EventPlayerMove eventPlayerMove) {
        if (this.Field3473.getValue() == Class303.VIRTUE && (Globals.mc.player.moveForward != 0.0f || Globals.mc.player.moveStrafing != 0.0f && !Class30.Method1835() && !Class30.Method1834())) {
            ...
            if (!(Globals.mc.player.collidedVertically || list.size() <= 0 && list2.size() <= 0)) {
                ...
                eventPlayerMove.cancel();
                eventPlayerMove.getDoubVal5(-0.001);
            }
            ++this.Field3477;
    ...
    private double Method4232(EntityPlayer entityPlayer, double d) {
        ...
        double d2 = 0.0;
        for (Object axisAlignedBB_ : list) {
            if (axisAlignedBB_ instanceof AxisAlignedBB) {
                AxisAlignedBB axisAlignedBB = (AxisAlignedBB) axisAlignedBB_;
    ...
    private void Method4218(EventPlayerMove eventPlayerMove) {
        if (eventPlayerMove.era != Class53.PRE) {
            return;
        }
    ...
    private void Method4217(EventNetworkPrePacketEvent eventNetworkPrePacketEvent) {
        if (((Boolean)this.Field3474.getValue()).booleanValue() && eventNetworkPrePacketEvent.getPacket() instanceof SPacketPlayerPosLook && this.isEnabled()) {
            this.enable();
        }
    }
}
